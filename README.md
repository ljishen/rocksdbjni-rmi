...
```bash
wget https://oss.sonatype.org/service/local/repositories/releases/content/com/github/ljishen/rocksdbjni-rmi/0.0.0.7/rocksdbjni-rmi-0.0.0.7.jar
java -jar rocksdbjni-rmi-0.0.0.7.jar [port] [hostname]
```
...
java -jar target/rocksdbjni-rmi-0.0.0.8-SNAPSHOT.jar [port] [hostname]
```
