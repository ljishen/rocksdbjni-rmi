...
```bash
wget https://oss.sonatype.org/service/local/repositories/releases/content/com/github/ljishen/rocksdbjni-rmi/0.0.0.8/rocksdbjni-rmi-0.0.0.8.jar
java -jar rocksdbjni-rmi-0.0.0.8.jar [port] [hostname]
```
...
java -jar target/rocksdbjni-rmi-0.0.0.9-SNAPSHOT.jar [port] [hostname]
```
